...
class AlbumsAndSongsController {
  ...
  async getAlbumsByPopularity(offset = 0, limit = 10) {
    try {
      console.log(
        `Consultando álbumes por popularidad en la base de datos local con offset: ${offset}, limit: ${limit}...`
      );
      ...
      const responseAlbums = await Promise.all(
        albumsFromDB.map(async (album) => {
          const songDetails = await Promise.all(
            album.idSong.map(async (song) => {
              const songDetail = await Songs.findById(song._id)
                .select("url_cancion")
                .exec();
              return {
                ...song.toObject(),
          ...
          const artists = await Promise.all(
            album.idArtist.map((artist) => artist.name)
          );

          return {
            name: album.name,
            duration:
              songDetails.reduce((acc, song) => acc + song.duration, 0) /
            ...
            songs: songDetails.map((song) => {
              if (!song.url_cancion) {
                console.error(
                  `Missing url_cancion for song: ${song.name}, album: ${album.name}`
                );
              }
              return {
                _id: song._id,
  ...
  async getSongsByGenres(offset = 0, limit = 10) {
    ...
    const genreImages = {
      ...
      reggaeton:
        "https://firebasestorage.googleapis.com/v0/b/spiralmusicapp.appspot.com/o/8e5504546937558f711958b221eafb3b7d60be76.jpeg?alt=media&token=62dc74c7-10f4-4778-b926-e651db39d7ee",
      indie:
        "https://firebasestorage.googleapis.com/v0/b/spiralmusicapp.appspot.com/o/652d7ed5bbf4223960b5c92f_What%20is%20Indie%20Music.jpg?alt=media&token=728fbd76-a460-4f94-aa00-b05f09957aa0",
      ...
      "dance pop":
        "https://firebasestorage.googleapis.com/v0/b/spiralmusicapp.appspot.com/o/musica-electronica-5.webp?alt=media&token=dcadfa8b-3ca4-4c05-9324-0290f8eb4465",
    };
  ...
  async getArtistsByPopularity(offset = 0, limit = 10) {
    try {
      console.log(
        `Consultando artistas por popularidad en la base de datos local con offset: ${offset}, limit: ${limit}...`
      );
      ...
      const responseArtists = await Promise.all(
